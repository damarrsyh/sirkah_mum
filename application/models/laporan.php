...
class Laporan extends GMN_Controller {
	...
	function proses_api_debitur(){
		...
		$get = $this->model_laporan->get_debitur($kreditur_code);
		...
		for($i = 0; $i < $count; $i++){
			$fields[$i]['no_id_cabang'] = $get[$i]['branch_code'];
			$fields[$i]['nm_anggota'] = $get[$i]['nama'];
			$fields[$i]['rembuk'] = $get[$i]['cm_name'];
			$fields[$i]['desa'] = $get[$i]['desa'];
			$fields[$i]['tgl_cair'] = $get[$i]['tanggal_akad'];
			$fields[$i]['pembiayaan'] = $get[$i]['pokok'];
			$fields[$i]['tenor'] = $get[$i]['jangka_waktu'];
			$fields[$i]['kd_cab'] = $get[$i]['branch_code'];
			$fields[$i]['keterangan'] = $get[$i]['description'];
			$fields[$i]['sektor'] = $get[$i]['sektor_ekonomi'];
			$fields[$i]['barang'] = $get[$i]['peruntukan'];
			$fields[$i]['no_ktp'] = $get[$i]['no_ktp'];
			$fields[$i]['tmp_lahir'] = $get[$i]['tmp_lahir'];
			$fields[$i]['tgl_lahir'] = $get[$i]['tgl_lahir'];
			$fields[$i]['alamat_lengkap'] = $get[$i]['alamat'];
			$fields[$i]['status'] = $get[$i]['city'];
			$fields[$i]['kabupaten'] = $get[$i]['city_abbr'];
			$fields[$i]['kode_pos'] = $get[$i]['kodepos'];
			$fields[$i]['ibu_kandung'] = $get[$i]['ibu_kandung'];
			$fields[$i]['cabang'] = $get[$i]['branch_name'];
			$fields[$i]['tahap'] = '1';
			$fields[$i]['statusa'] = '1';
		}

		/*
		$ch2 = curl_init('https://bprshasanah.co.id/apimum/api/debitur');

		curl_setopt_array($ch2,array(
			CURLOPT_POST => TRUE,
			CURLOPT_RETURNTRANSFER => TRUE,
			CURLOPT_HTTPHEADER => array('Content-Type: application/json','Connection: Keep-Alive','key: '.$key),
		...
		$ret = json_decode($response2,TRUE);
		...
		*/
		...
		$ch = curl_init('https://bprshasanah.co.id/apimum/api/debitur');
        ...
        try {
        	...
        	for($i = 2; $i < ($numRows + 1); $i++){
        		...
        		if( $i !=$numRows) 
       			 ...
       			 else { $norek_array=$norek_array.$norek."')"; }  
  ...
  //       print_r($count);
    ...
    */
	...
	function jqgrid_list_pelunasan_pembiayaan(){
		...
		$count = $this->model_laporan_to_pdf->jqgrid_count_pelunasan_pembiayaan($cabang,$pembiayaan,$petugas,$majelis,$tanggal,$tanggal2,$kreditur
			);
	...
	function jqgrid_list_jatuh_tempo(){
		...
		$count = $this->model_laporan_to_pdf->jqgrid_count_jatuh_tempo($cabang,$pembiayaan,$petugas,$majelis,$tanggal,$tanggal2
			);
	...
	function jqgrid_list_kolektibilitas(){
		...
		$count = $this->model_laporan_to_pdf->jqgrid_count_kolektibilitas($tanggal_hitung,$branch_code,$kol);
		...
		$result = $this->model_laporan_to_pdf->jqgrid_list_kolektibilitas($sidx,$sort,$limit_rows,$start,$tanggal_hitung,$branch_code,$kol);
	...
	function jqgrid_list_saldo_tabungan(){
		...
		$count = $this->model_laporan_to_pdf->jqgrid_count_saldo_anggota($cabang,$petugas,$majelis
			);
	...
	function jqgrid_list_anggota_masuk(){
		...
		$count = $this->model_laporan_to_pdf->jqgrid_count_anggota_masuk($cabang,$majelis,$tanggal,$tanggal2
			);
	...
	function jqgrid_list_anggota_keluar(){
		...
		$count = $this->model_laporan_to_pdf->jqgrid_count_anggota_keluar($cabang,$majelis,$tanggal,$tanggal2,$alasan
			);
	...
	function jqgrid_list_anggota_mutasi(){
		...
		$count = $this->model_laporan_to_pdf->jqgrid_count_anggota_mutasi($cabang,$majelis,$tanggal,$tanggal2,$alasan
			);
	...
	function jqgrid_list_buka_tabungan(){
		...
		$count = $this->model_laporan_to_pdf->jqgrid_count_list_buka_tabugan($produk,$tanggal,$tanggal2,$cabang
			);
	...
	function jqgrid_list_buka_tabungan_jtempo(){
		...
		$count = $this->model_laporan_to_pdf->jqgrid_count_list_buka_tabugan_jtempo($produk,$tanggal,$tanggal2,$cabang,$status,$rembug
			);
	...
	function jqgrid_list_pembukaan_tabungan(){
		...
		$count = $this->model_laporan_to_pdf->jqgrid_count_list_pembukaan_tabungan($produk,$cabang
			);
