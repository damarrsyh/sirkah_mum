		...
		*/
		...
		if ($debug==true) {
			...
			if($confirm == TRUE){
				...
				$this->model_transaction->update_no_hp($datasa);
				$this->model_transaction->update_p_no_hp($datasas);
...
/*
	...
	public function verifikasi_rek_pembiayaan()
	{
		...
		if($financing_type == 1){
			...
			$d_saving = array('account_saving_no' => $cif_no);
	...
	public function search_account_saving_no()
	{
		...
		$data = $this->model_transaction->account_saving_no_individu($keyword,$cif_type,$cm_code,$branch);
	...
	public function search_cif_by_account_saving()
	{
		...
		$cm_code = $this->input->post('cm_code');
		$branch_code = $this->session->userdata('branch_code');
		$data = $this->model_transaction->account_saving_no_individu($keyword,$type,$cm_code,$branch_code);
			...
			*/
			...
			for ( $y=0; $y<count($cif_no); $y++ )
			{
				...
				*/
				...
				/*
				| INSERTING DATA LWK INTO mfi_trx_cm_lwk
				| @Condition : if $setoran_lwk is not 0
				| BEGIN
				*/
				if($setoran_lwk[$y]>0){
				...
				*/
				...
				if( count($get_financing) > 0 ){
					...
					*/
					...
					| BEGIN
					*/
					$get_financing_droping = $this->model_transaction->get_account_financing_droping($get_financing['account_financing_no'],$tanggal);
					if($get_financing_droping['jumlah'] > 0){
...
/*
	...
	*/
	...
	public function aktivasi_transaksi()
	{
		...
		if($jenis=="Pembiayaan"){
			...
			/* get account cash code by gl code
			$get_account_cash_code = $this->model_transaction->get_account_cash_code($account_cash_code);
			$account_cash_teller_code = $get_account_cash_code['account_cash_code'];
			*/
	...
	public function aktivasi_transaksi_old()
	{
		$id 	= $this->input->post('id');
		$jenis 	= $this->input->post('jenis');
		$account_no = $this->input->post('account_no');
	...
	*/
	...
	public function  do_perpanjang_tabber()
	{
			...
			// INSERT TO account_saving_schedulle

			$account_saving_no 					= $this->input->post('account_saving_no');
			$rencana_jangka_waktu_sebelum 		= $this->input->post('rencana_jangka_waktu');
			$rencana_jangka_waktu_setelah 		= $this->input->post('rencana_jangka_waktu2');
			
			$tanggal_perpanjangan				= $this->input->post('tanggal_perpanjangan');
        	$tanggal_perpanjangan_new 			= substr($tanggal_perpanjangan,4,4).'-'.substr($tanggal_perpanjangan,2,2).'-'.substr($tanggal_perpanjangan,0,2);
			
			$counter_angsruan 					= $this->input->post('counter_angsruan');

			// $data=array('status_verif'=>'1');

			$datas = array(
				'account_saving_no' 			=> $account_saving_no,
				'rencana_jangka_waktu_sebelum' 	=> $rencana_jangka_waktu_sebelum,
				'rencana_jangka_waktu_setelah' 	=> $rencana_jangka_waktu_setelah,
				'tanggal_perpanjangan' 			=> $tanggal_perpanjangan_new,
			...
			// END INSERT account_saving_schedule

			$param = array('account_saving_no'	=>$account_saving_no);

			$this->db->trans_begin();
			$this->model_transaction->insert_perpanjang_tabber($datas,$param);
			
			if($this->db->trans_status()===true){
				$this->db->trans_commit();
				$return = array('success'=>true,'message'=>'Success: Perpanjang Tabungan Berencana Berhasil!');
			}else{
				$this->db->trans_rollback();
				$return = array('success'=>false,'message'=>'Error: Failed to connect into databases, please try again latter or contact your administrator!');
			}
	...
	public function verif_perpanjang_tabber(){
			$account_saving_no 					= $this->input->post('account_saving_no');
			// UPDATE TO  mfi_account_saving

				$rencana_jangka_waktu			= $this->input->post('rencana_jangka_waktu_setelah');
			...
			if($this->db->trans_status()===true){
				$this->db->trans_commit();
				$return = array('success'=>true,'message'=>'Success: Verifikasi Tabungan Berencana Berhasil!');
			}else{
				$this->db->trans_rollback();
				$return = array('success'=>false,'message'=>'Error: Failed to connect into databases, please try again latter or contact your administrator!');
			}
