...
class Rekening_nasabah extends GMN_Controller { 
	...
	public function proses_reg_pelunasan_pembayaran()
	{
		...
		$tanggal_transaksi				= date('Y-m-d',strtotime(str_replace('/','-',$this->input->post('tanggal_lunas'))));
		$trx_detail_id 					= uuid(false);

		$now = $jtempo_angsuran_next;
		$jangkawaktu = $periode_jangka_waktu; // 0 = harian, 1=mingguan, 2=bulanan, 3=jtempo
		$jtempo_db = $tanggal_jtempo;

		$jtempo = null;
		if($jangkawaktu==0)
		$jtempo = date("Y-m-d",strtotime($now.' +1 days'));
		else if($jangkawaktu==1)
		$jtempo = date("Y-m-d",strtotime($now.' +7 days'));
		else if($jangkawaktu==2)
		$jtempo = date("Y-m-d",strtotime($now.' +1 month'));
	...
	public function datatable_verifikasi_pelunasan_pembiayaan()
	{
		...
		{
			$sWhere = "where mafl.status_pelunasan ='0'";
		}
	...
	function proses_verifikasi_pelunasan_pembayaran(){
		...
		$data_acc_financing		= array(
			...
			'saldo_pokok'		=>'0',
			'saldo_margin'		=>'0',
			'saldo_catab'		=>$saldo_kalkulasi_catab
		);
		...
		$this->model_nasabah->update_trx_account_financing($data_trx_account_financing,$param_trx_financing);
	...
	public function datatable_pengajuan_pembiayaan_setup()
	{
		...
		{
			if($aRow['status']==0){
			  $aRow['status'] = '<center><a href="javascript:;" account_financing_reg_id="'.$aRow['account_financing_reg_id'].'" id="link-edit">Edit</a></center>';
	...
	public function edit_pengajuan_pembiayaan()
	{
        ...
        $rencana_droping 	= substr($rencana_droping_,4,4).'-'.substr($rencana_droping_,2,2).'-'.substr($rencana_droping_,0,2);
			...
			$data = array(
				...
				,'description'			=>$description
				,'created_by'			=>$created_by
				,'financing_type'		=>$financing_type2
				,'tanggal_pengajuan'	=>$tanggal_pengajuan
				,'pembiayaan_ke'		=>$pyd
				,'uang_muka'			=>$this->convert_numeric($uang_muka)
			);	
	...
	public function verifikasi_rescheduling()
	{
	...
	public function proses_pencairan_tabungan()
	{
		...
		$data_trx_account_saving = array(
				...
				,'description' => 'pencairan dan penutupan tabungan no. rek:'.$account_saving_no.' a.n '.$nama
				,'trx_status' => 0
				,'trx_detail_id' => $trx_detail_id
			...
			if($cif_type=='0'){ // tabungan berencana
				...
				if($jumlah_penarikan>0){
					$data_trx_sukarela[] = array(
						...
						,'trx_type'=>4
						,'flag_debet_credit'=>'C'
						,'trx_source_id'=>$trx_account_saving_id
	...
	*/
	...
	public function search_account_financing_no()
	{
		...
		$data = $this->model_nasabah->search_account_financing_no($keyword,$cm_code,$status_rekening);
	...
	*/
	...
	function approve_pembukaan_tabungan()
	{
		...
		// $saving = $this->model_nasabah->get_account_saving_by_id($account_saving_id);
		...
		$this->db->trans_begin();
		$this->db->insert('mfi_trx_gl_cash',$data_kas);
		$this->db->update('mfi_account_saving',$data,$param);
		$this->db->query("select fn_jurnal_biaya_adm_tab_berencana(?,?)",array($account_saving_id,$kas_petugas));
	...
	*/
		    ...
		    if($bValid==true){
		    	...
		    	$raw_log_koreksi_droping = array(
		    		...
		    		,'pokok' => $pokok
						...
						,'angsuran_tab_wajib' => $angsuran_tab_wajib
						,'angsuran_tab_kelompok' => $angsuran_tab_kelompok
						...
						,'angsuran_tab_wajib2' => $angsuran_tab_wajib2
						,'angsuran_tab_kelompok2' => $angsuran_tab_kelompok2
	...
	public function perpanjang_tabber()
	{
	...
	{
		$account_saving_no 				= $this->input->post('account_saving_no');
		$data 							= $this->model_nasabah->get_value($account_saving_no);
	...
	public function verifikasi_perpanjang_tabber()
	{
