...
class Laporan_to_pdf extends GMN_Controller {
    ...
    function export_keuangan_labarugi_bulanan(){
        ...
        $data['report_item'] = $this->model_laporan_to_pdf->export_keuangan_labarugi_bulanan($cabang,$last_date,$report_code);
    ...
    function export_keuangan_neraca_bulanan(){
        ...
        $config['full_tag_open'] = '<p>';
        $config['full_tag_close'] = '</p>';
        $data['result'] = $this->model_laporan_to_pdf->export_keuangan_neraca_bulanan($branch_code,$last_date,$report_code);
        if ($branch_code !='00000'){
            $data['branch_name'] = $this->model_laporan_to_pdf->get_cabang($branch_code);
        } else {
            $data['branch_name'] = "PUSAT";
            ...
            if($i==0)
            {
                ...
                $html .= '
                     <tr>
                        ...
                        <td width="230" align="center" style="border:solid 1px #999;border-bottom:solid 0px #FFF;border-right:solid 1px #FFF;" colspan="5">Setoran</td>
    ...
    public function export_list_buka_tabungan_jtempo()
    {
        ...
        $data['saldo_tabungan'] = $this->model_laporan->export_list_buka_tabungan_jtempo($produk,$from_date,$thru_date,$branch_code);
        $data['product_name']   = $this->model_laporan->get_produk($produk);
        $data['produk']         = $produk;
        $data['tanggal1_view']  = $from_date;
        $data['tanggal2_view']  = $thru_date;
            ...
            if (isset($datas['data']['nama'])) 
            {
                ...
                foreach($datass as $data)
                {
                    ...
                    for ($i=0;$i<$data['jangka_waktu'];$i++) { //loop
                        ...
                        // $historycm=$this->model_laporan->get_history_cm_trx_date($cif_no,$tgl_angsur);
    ...
    */
    ...
    public function neraca_saldo_gl()
    {
        ...
        $data['periode1'] = date('d-m-Y',strtotime($periode1));
        $data['periode2'] = date('d-m-Y',strtotime($periode2));
...
*/
    ...
    function rekap_trx_rembug() {
        ...
        $data_rembug = $this->model_laporan->get_rembug_by_fa_code_hari($fa_code,$hari);
        ob_start();
    ...
    */
    ...
    public function export_rekap_angsuran_semua_cabang()
    {
        ...
        try
        {
            ...
            $html2pdf->pdf->SetDisplayMode('fullpage');
            $html2pdf->writeHTML($content, isset($_GET['vuehtml']));
            $html2pdf->Output('REKAP ANGSURAN.pdf');
        }
        catch(HTML2PDF_exception $e) {
            echo $e;
            exit;
    ...
    public function export_rekap_angsuran_rembug()
    {
        ...
        try
        {
            ...
            $html2pdf->pdf->SetDisplayMode('fullpage');
            $html2pdf->writeHTML($content, isset($_GET['vuehtml']));
            $html2pdf->Output('REKAP ANGSURAN BY REMBUG.pdf');
        }
        catch(HTML2PDF_exception $e) {
            echo $e;
            exit;
    ...
    public function export_rekap_angsuran_petugas()
    {
		...
		try
		{
			...
			$html2pdf->pdf->SetDisplayMode('fullpage');
			$html2pdf->writeHTML($content, isset($_GET['vuehtml']));
			$html2pdf->Output('REKAP ANGSURAN BY PETUGAS.pdf');
		}
		catch(HTML2PDF_exception $e) {
			echo $e;
			exit;
    ...
    public function export_rekap_angsuran_produk()
    {
		...
		try
		{
			...
			$html2pdf->pdf->SetDisplayMode('fullpage');
			$html2pdf->writeHTML($content, isset($_GET['vuehtml']));
			$html2pdf->Output('REKAP ANGSURAN BY PRODUK.pdf');
		}
		catch(HTML2PDF_exception $e) {
			echo $e;
			exit;
    ...
    public function export_rekap_angsuran_peruntukan()
    {
		...
		try
		{
			...
			$html2pdf->pdf->SetDisplayMode('fullpage');
			$html2pdf->writeHTML($content, isset($_GET['vuehtml']));
			$html2pdf->Output('REKAP ANGSURAN BY PERUNTUKAN.pdf');
		}
		catch(HTML2PDF_exception $e) {
			echo $e;
			exit;
    ...
    public function export_rekap_angsuran_sektor_usaha()
    {
        ...
        try
        {
            ...
            $html2pdf->pdf->SetDisplayMode('fullpage');
            $html2pdf->writeHTML($content, isset($_GET['vuehtml']));
            $html2pdf->Output('REKAP ANGSURAN BY SEKTOR USAHA.pdf');
        }
        catch(HTML2PDF_exception $e) {
            echo $e;
            exit;
