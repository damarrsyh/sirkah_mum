    ...
    $("#select").click(function(){
        ...
        $.ajax({
          ...
          success: function(response){
			  var total_angsuran_catab = (response.jangka_waktu - response.counter_angsuran) * response.angsuran_catab;
               ...
               $("#saldo_tabungan").val(number_format(total_angsuran_catab,0,',','.'));
               $("#no_rek_tabungan").val(response.account_saving_no);
			   $("#saldo_memo").val(number_format(response.saldo_memo,0,',','.'));
			   $('#potongan_margin').val('0');
			   var saldo_pokok = parseFloat(convert_numeric(response.saldo_pokok));
			   var saldo_margin = parseFloat(convert_numeric(response.saldo_margin));
			   var saldo_tabungan = parseFloat(convert_numeric(total_angsuran_catab));
			   var total_pembayaran = (saldo_pokok + saldo_margin + saldo_tabungan) - parseFloat($('#potongan_margin').val());
        ...
        $("#button-dialog").click(function(){
		  ...
		  $('#calculate_saldo_catab').click(function(){
			  ...
			  var saldo_tabungan = parseFloat(convert_numeric($("#saldo_tabungan").val()));

			  if(f_cek){
				  var total_pembayaran = (saldo_pokok + saldo_margin) - parseFloat(convert_numeric($('#potongan_margin').val()));
			  } else {
				  var total_pembayaran = (saldo_pokok + saldo_margin + saldo_tabungan) - parseFloat(convert_numeric($('#potongan_margin').val()));
			  }
       ...
       $("#potongan_margin","#form_add").change(function(){
		 ...
		 var saldo_tabungan = parseFloat(convert_numeric($("#saldo_tabungan","#form_add").val()));
		 
		 if(f_cek){
			 var total_pembayaran = saldo_pokok + saldo_margin - margin;
		 } else {
			 var total_pembayaran = saldo_pokok + saldo_margin + saldo_tabungan - margin;
		 }
