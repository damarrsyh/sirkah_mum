...
class Model_laporan extends CI_Model {
	...
	function get_history_cm_trx_date_by_account_financing_no($account_financing_no,$angsuran_ke,$financing_type,$jtempo){
		...
		if($financing_type == '0'){
			...
			LEFT JOIN mfi_user AS b ON b.user_id::VARCHAR = a.verify_by::VARCHAR
	...
	public function export_list_transaksi_rembug_sub($trx_cm_id,$from_date,$thru_date,$trx_date='')
	{
		$sql = "
				...
				(select coalesce(sum(b.amount*b.freq),0)
					from mfi_trx_cm_detail_savingplan a, mfi_trx_cm_detail_savingplan_account b
					where a.trx_cm_detail_savingplan_id=b.trx_cm_detail_savingplan_id and
	...
	public function export_list_buka_tabungan($produk,$from_date,$thru_date,$branch_code)
	{
		...
		$sql .= " ORDER BY mfi_account_saving.tanggal_buka ASC";
	...
	public function export_list_buka_tabungan_jtempo($produk,$from_date,$thru_date,$branch_code)
	{
		$param = array();
		$sql = "SELECT 
				mps.*,
	...
	function get_tanggal_closing($branch_code){
		$sql = "SELECT closing_from_date, closing_thru_date FROM mfi_closing_ledger_data_2 GROUP BY 1,2 ORDER BY closing_thru_date DESC";

		$param = array();

		if($branch_code != '00000'){
			$sql ="SELECT closing_from_date, closing_thru_date 
					FROM mfi_closing_ledger_data 
					WHERE branch_code IN(SELECT branch_code FROM mfi_branch_member WHERE branch_induk = ?) 
					GROUP BY 1,2 ORDER BY closing_thru_date DESC";
			$param[] = $branch_code;
		}
	...
	{
		$sql = "select a.account_saving_no,b.product_name from mfi_account_saving a,mfi_product_saving b where a.product_code=b.product_code and a.cif_no=?";
		$query = $this->db->query($sql,array($cif_no));
		return $query->result_array();
	}
	...
	function get_saldo_awal_tab_wajib_kelompok($cif_no='',$from_date='')
	{
		...
		$sql = "SELECT SUM(a.tab_wajib_cr*a.freq) AS s
				...
				WHERE date(b.created_date) < ? AND a.cif_no=? ";
		$param[]=$from_date;
		$param[]=$cif_no;
		$query = $this->db->query($sql,$param);
	...
	function get_statement_tab_kelompok_tab_wajib($cif_no='',$from_date='',$thru_date='')
	{
		...
		$sql = "SELECT date(b.created_date) created_date, a.tab_wajib_cr,a.freq
				FROM mfi_trx_cm_detail a
				INNER JOIN mfi_trx_cm b ON b.trx_cm_id=a.trx_cm_id
				WHERE date(b.created_date) between ? AND ? AND a.cif_no=? ORDER BY 1 ";
		$param[]=$from_date;
		$param[]=$thru_date;
		$param[]=$cif_no;
		$query = $this->db->query($sql,$param);
	...
	function get_statement_tab_kelompok_tab_sukarela($cif_no='',$from_date='',$thru_date='')
	{
		...
		$sql = "select 
				...
				(case when trx_type='1' then 'TRX ANGGOTA KELUAR (TAB.WAJIB)'
				      ...
				      when trx_type='9' then 'BAGI HASIL SHU' end) as description,
	...
	function get_rembug_by_fa_code_hari($fa_code,$hari) {
		$sql = "select cm_code,cm_name from mfi_cm where fa_code=? and hari_transaksi=?";
		$query = $this->db->query($sql,array($fa_code,$hari));
		return $query->result_array();
	}
