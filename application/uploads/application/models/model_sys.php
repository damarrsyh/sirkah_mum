...
Class Model_sys extends CI_Model {
	...
	function fn_insert_closing_ledger_data($branch_code,$from_date_lm,$from_date,$thru_date,$closing_id,$user_id){
		$sql = "SELECT fn_insert_closing_ledger_data(?,?,?,?,?,?)";

		$param = array($branch_code,$from_date_lm,$from_date,$thru_date,$closing_id,$user_id);
	...
	*/
	...
	function insert_closing_balance_data($data)
	{
		$this->db->insert_batch('mfi_closing_balance_data',$data);
	}
	function insert_closing_financing_data($data)
	{
	...
	*/
	...
	function getBranchsGL()
	{
		$sql = "select a.branch_code,a.branch_name
				...
				where a.branch_code = b.branch_code group by 1,2 order by 1";
		$query = $this->db->query($sql);
		return $query->result_array();
	}
	...
	function get_amount_titipan_bagihasil($date1,$date2,$branch_code)
	{
		...
		$sql = "
			SELECT coalesce(sum(
					(case when a.flag_debit_credit='D' then a.amount*-1 else a.amount end)
				),0) amount
			FROM mfi_titipan_bagihasil a, mfi_cif b
			WHERE a.trx_date between ? and ?
			and a.cif_no=b.cif_no
		";*//*
		$sql = "
			SELECT coalesce(sum(
					(case when flag_debit_credit='D' then amount*-1 else amount end)
				),0) amount
			FROM mfi_titipan_bagihasil
			WHERE trx_date between ? and ?
		";*/
