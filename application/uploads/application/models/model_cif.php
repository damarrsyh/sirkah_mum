...
Class Model_cif extends CI_Model {
	...
	public function datatable_kantor_cabang_setup($sWhere='',$sOrder='',$sLimit='')
	{
		$sql = "SELECT
				...
				FROM
							mfi_branch
				INNER JOIN mfi_list_code_detail ON mfi_branch.branch_officer_title = CAST(mfi_list_code_detail.code_value as integer)
				WHERE mfi_list_code_detail.code_group='jabatan'
				 ";
	...
	public function get_branch_by_branch_id($branch_id)
	{
		$sql = "SELECT
				...
				FROM
							mfi_branch
				LEFT JOIN mfi_list_code_detail ON mfi_branch.branch_officer_title = CAST(mfi_list_code_detail.code_value as integer)
				WHERE mfi_list_code_detail.code_group='jabatan' AND mfi_branch.branch_id = ?";
	...
	public function search_cif_no($keyword='',$type='',$cm_code='')
	{
		...
		$sql = "SELECT cif_no,nama,cm_name from mfi_cif left join mfi_cm on mfi_cm.cm_code = mfi_cif.cm_code where (upper(mfi_cif.nama) like ? or mfi_cif.cif_no like ?) and status=1";
	...
	public function get_cm_data()
	{
		...
		$sql = "SELECT mfi_cm.cm_code, mfi_cm.cm_name, mfi_branch.branch_code 
				from mfi_cm, mfi_branch 
				WHERE mfi_branch.branch_id=mfi_cm.branch_id ";
		if ($branch_code!='00000') {
			$sql .= " AND mfi_branch.branch_code in(select branch_code from mfi_branch_member where branch_induk=?)";
			$param[] = $branch_code;
		}
		$sql.=" order by mfi_cm.cm_name asc";
	...
	// search cif number
	public function search_cif_by_account_no($keyword,$type,$cm_code)
	{
		$branch_code = $this->session->userdata('branch_code');
		$flag_all_branch = $this->session->userdata('flag_all_branch');
